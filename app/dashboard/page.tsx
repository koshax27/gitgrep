...
export default function Dashboard() {
  ...
  const [filter, setFilter] = useState("all"); // all, bug, feature, ui
  const [selectedFeedback, setSelectedFeedback] = useState<Feedback | null>(null);

  // جلب الفيدباك من localStorage (لأنه في مرحلة التطوير)
  useEffect(() => {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            <button
              onClick={exportData}
              className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-500 rounded-xl text-white text-sm transition-all"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
            ...
            <div className="text-sm text-white">
              🐛 {stats.categories.bug} | 💡 {stats.categories.feature} | 🎨 {stats.categories.ui}
            </div>
          </div>
        </div>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      ...
                      fb.category === 'feature' ? 'bg-green-500/20 text-green-400' :
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-[#0d1117] border border-white/10 rounded-2xl p-6 max-w-md w-full mx-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setSelectedFeedback(null)} className="p-1 hover:bg-white/10 rounded-lg">
                  <X size={18} />
                </button>
              </div>
              <div className="space-y-4">
                <div>
                  <span className="text-slate-400 text-xs uppercase">Rating</span>
                ...
                <div>
                  ...
                  <p className="text-white mt-1 capitalize">{selectedFeedback.category}</p>
                </div>
                <div>
                  <span className="text-slate-400 text-xs uppercase">Feedback</span>
                  <p className="text-white mt-1">{selectedFeedback.feedback}</p>
                </div>
                {selectedFeedback.email && (
                  <div>
                    <span className="text-slate-400 text-xs uppercase">Email</span>
                    <p className="text-white mt-1">{selectedFeedback.email}</p>
                  </div>
                )}
                <div>
