...
import {
  ...
  Filter, Sliders, RefreshCw, Save, FileJson, FileText
} from "lucide-react"
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      for (const key of keys) {
        if ((key === 'Ctrl+K' || key === 'Meta+K') && isCtrlOrCmd && e.key === 'k') {
          e.preventDefault();
          e.stopPropagation();
          callback();
          return;
        }
        if ((key === 'Ctrl+S' || key === 'Meta+S') && isCtrlOrCmd && e.key === 's') {
          e.preventDefault();
          e.stopPropagation();
          callback();
          return;
...
function ExportModal({ results, onClose }: { results: SearchResult[]; onClose: () => void }) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `gitgrep-results.${format}`;
    a.click();
...
export default function Home() {
  ...
  useKeyboardShortcut(['Ctrl+S', 'Meta+S'], () => {
    if (results.length > 0) setShowExportModal(true);
  });
          ...
          <>
            ...
            <div className="max-w-4xl mx-auto mb-8">
              <div className="bg-[#0d1117] border border-white/10 rounded-2xl overflow-visible">
                ...
                <div className="flex items-center justify-between gap-4 px-4 py-3 bg-white/5">
                  <div className="relative">
                    <button onClick={() => document.getElementById('filter-dropdown')?.classList.toggle('hidden')} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-slate-900 dark:text-white text-sm">
                      <Filter size={14} />
                      <span>Filters</span>
                      <ChevronDown size={12} />
                    </button>
                    
                    <div id="filter-dropdown" className="hidden absolute top-full left-0 mt-2 w-80 bg-[#0d1117] border border-gray-700 rounded-2xl p-5 z-[100] shadow-2xl">
                      <div className="space-y-5">
                        <div>
                          ...
                          <input type="number" id="minStars" placeholder="0" className="w-full bg-black/50 border border-gray-700 rounded-xl p-3 text-slate-900 dark:text-white focus:border-blue-500 outline-none" />
                        </div>
                        ...
                        <div>
                          ...
                          <select id="language" className="w-full bg-black/50 border border-gray-700 rounded-xl p-3 text-slate-900 dark:text-white focus:border-blue-500 outline-none cursor-pointer">
                  ...
                  <button onClick={search} disabled={loading} className="bg-blue-600 hover:bg-blue-500 text-slate-900 dark:text-white px-8 py-2.5 rounded-xl font-bold text-sm">
            ...
            <div className="space-y-12 mt-12">
              <div className="flex items-center justify-between">
                ...
                {results.length > 0 && (
                  <div className="flex gap-2">
                    <button onClick={() => setShowExportModal(true)} className="p-2 hover:bg-white/10 rounded-lg" title="Export">
                      <Download size={16} />
                    </button>
                    <button onClick={() => setShowShareModal(true)} className="p-2 hover:bg-white/10 rounded-lg" title="Share">
                      <Share2 size={16} />
                    </button>
                  </div>
                )}
